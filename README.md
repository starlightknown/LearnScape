...
## 🔧 How we built it-
1. Developed an API to get all the projects in the database and specific project from large project data. The project id is given by the database. This API will also give you the number of projects provided in limit field and you can also increment the page numbers to get the next limit frame data along with all names of the projects. Check out the documentation of API [here](https://github.com/starlightknown/LearnScape/tree/main/api)
2. The fronted/client consumes this API in order to fetch the details of several CNCF projects which can be needed by the beginner.
